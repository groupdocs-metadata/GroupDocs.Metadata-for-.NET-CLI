...
namespace GroupDocs.Metadata.Cli.Common.Commands
{
    ...
    internal class RemoveCommand : ICommand
    {
        ...
        /// </summary>
        public CommandType CommandType { get => CommandType.Remove; }
        ...
        public void Execute(string[] args = null)
        {
            ...
            try
            {
                var count = All ? metadata.Sanitize() : metadata.RemoveProperties(p => p.Name == PropertyName);

                metadata.Save();

                metadata.Dispose();

                Reporter.Output.WriteLine($"Remove {count} properties.");
            }
            catch (Exception e)
            {
        ...
        public CommandLineParseResult Parse(string[] args)
        {
            ...
            if (thisCommandArguments.Length < 1)
            {
                return CommandLineParseResult.Failure(
                    $"{MainCommands.RemoveCommand} command should have at least 1 argument");
            }
            ...
            if(All == false && string.IsNullOrEmpty(PropertyName))
                return CommandLineParseResult.Failure(
                    $"{MainCommands.RemoveCommand} command should have a -all flag or -name");
