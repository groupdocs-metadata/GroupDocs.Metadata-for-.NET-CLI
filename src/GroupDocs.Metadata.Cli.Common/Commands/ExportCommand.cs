...
{
    /// <summary>
    /// Export document to CSV/JSON/XML/XLS/XLSX command implementation.
    /// </summary>
    internal class ExportCommand : ICommand
    {
        ...
        /// </summary>
        private string DestinationFileName { get; set; } = "output.xml";
        ...
        /// <summary>
        /// Output format to view (render) - By default we view document to XML.
        /// </summary>
        private OutputFormat OutputFormat { get; set; } = OutputFormat.Xml;
        ...
        /// </summary>
        public CommandType CommandType { get => CommandType.Export; }
        ...
        public void Execute(string[] args = null)
        {
            ...
            try
            {
                ...
                {
                    manager.Export(DestinationFileName, ExportFormat.Xml);
                }
                else if (OutputFormat == OutputFormat.Csv)
                {
                    manager.Export(DestinationFileName, ExportFormat.Csv);
                }
                else if (OutputFormat == OutputFormat.Json)
                {
                    manager.Export(DestinationFileName, ExportFormat.Json);
                }
                else if (OutputFormat == OutputFormat.Xls)
                {
                    manager.Export(DestinationFileName, ExportFormat.Xls);
                }
                else if (OutputFormat == OutputFormat.Xlsx)
                {
                    manager.Export(DestinationFileName, ExportFormat.Xlsx);
                }
        ...
        public CommandLineParseResult Parse(string[] args)
        {
            ...
            if (thisCommandArguments.Length < 1)
            {
                return CommandLineParseResult.Failure(
                    $"{MainCommands.ExportCommand} command should have at least 1 argument");
            }
